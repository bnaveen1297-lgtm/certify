...
function elIcon(type: string) {
  switch (type) {
    ...
    case "logo_organizer":
    case "logo_sponsor":  return <ImageIcon size={10} className="text-white/40" />
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="border-b border-white/10">
        ...
        >
          <span>Borders &amp; Frames ({BORDER_PRESETS.length})</span>
        ...
        <div className="flex flex-wrap gap-1">
          {TEXT_TOKENS.map(t => (
            <button
              key={t.token}
              title={t.label}
              onClick={() => onAddElement(defaultTextElement({ content: t.token, fontSize: 14, color: "#475569" }))}
