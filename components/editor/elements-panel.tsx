...
import {
  ...
  Eye, EyeOff, Lock, Unlock, Trash2, Copy, GripVertical, ChevronDown, ChevronRight,
} from "lucide-react"
            ...
            return (
              ...
              >
                ...
                <div className={`flex items-center gap-0.5 ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity`}>
                  <button onClick={e => { e.stopPropagation(); onToggleVisibility(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-white rounded" title={el.visible ? "Hide" : "Show"}>
                    {el.visible ? <Eye size={11} /> : <EyeOff size={11} />}
                  </button>
                  <button onClick={e => { e.stopPropagation(); onToggleLock(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-white rounded" title={el.locked ? "Unlock" : "Lock"}>
                    {el.locked ? <Lock size={10} /> : <Unlock size={10} />}
                  </button>
                  <button onClick={e => { e.stopPropagation(); onDuplicateElement(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-white rounded" title="Duplicate">
                    <Copy size={10} />
                  </button>
                  <button onClick={e => { e.stopPropagation(); onRemoveElement(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-red-400 rounded" title="Delete">
                    <Trash2 size={10} />
                  </button>
                </div>
              </div>
