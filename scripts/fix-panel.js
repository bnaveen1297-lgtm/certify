...
function elLabel(el: TemplateElement): string {
  ...
  const map: Record<string, string> = {
    ...
    logo_organizer: "Organizer Logo", logo_sponsor: "Sponsor Logo",
  }
...
export function ElementsPanel({
  ...
  function handleDragEnd() {
    ...
    setDragIdx(null); setOverIdx(null); dragItem.current = null; dragOver.current = null
    if (from === null || to === null || from === to) return
    const reordered = [...sorted]
    const [moved] = reordered.splice(from, 1)
    reordered.splice(to, 0, moved)
    const maxZ = reordered.length
    onReorderElements(reordered.map((el, i) => ({ ...el, zIndex: maxZ - i })))
  }
    ...
    <div className="flex flex-col">
      ...
      <div className="p-3">
        ...
        {template.elements.length === 0 && (
          <p className="text-[11px] text-white/20 text-center py-4">No elements yet.<\/p>
        )}
          ...
          {sorted.map((el, idx) => {
            ...
            return (
              <div
                ...
                className={\`group flex items-center gap-1.5 px-2 py-1.5 rounded cursor-pointer select-none transition-all \${isSelected ? "bg-[#3b82f6]/15 border border-[#3b82f6]/40" : "hover:bg-white/5 border border-transparent"} \${isDragging ? "opacity-40" : "opacity-100"} \${isOver ? "border-t-2 border-t-[#3b82f6]" : ""}\`}
              >
                <span className="text-white/20 hover:text-white/50 cursor-grab active:cursor-grabbing shrink-0"><GripVertical size={12} /><\/span>
                <span className="shrink-0 w-3.5 flex items-center justify-center">{elIcon(el.type)}<\/span>
                <span className={\`flex-1 text-[11px] truncate \${el.visible ? "text-white/70" : "text-white/25 line-through"}\`}>{elLabel(el)}<\/span>
                <div className={\`flex items-center gap-0.5 \${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity\`}>
                  <button onClick={e => { e.stopPropagation(); onToggleVisibility(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-white rounded">
                    {el.visible ? <Eye size={11} /> : <EyeOff size={11} />}
                  <\/button>
                  <button onClick={e => { e.stopPropagation(); onToggleLock(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-white rounded">
                    {el.locked ? <Lock size={10} /> : <Unlock size={10} />}
                  <\/button>
                  <button onClick={e => { e.stopPropagation(); onDuplicateElement(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-white rounded">
                    <Copy size={10} />
                  <\/button>
                  <button onClick={e => { e.stopPropagation(); onRemoveElement(el.id) }} className="w-5 h-5 flex items-center justify-center text-white/30 hover:text-red-400 rounded">
                    <Trash2 size={10} />
                  <\/button>
                <\/div>
              <\/div>
...
console.log('Has duplicate ADD_ITEMS:', (written.match(/const ADD_ITEMS/g) || []).length)
console.log('Done.')
